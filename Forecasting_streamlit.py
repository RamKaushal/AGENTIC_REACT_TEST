...
# citi_sci_forecast_app_v3.py
# Streamlit "Forecast Command Centre" — Citibank-inspired + Futuristic Sci‑Fi Chat Interface
# STRICT GATED FLOW + RIGHT-SIDE ANCHORED ROWS
# To run: streamlit run citi_sci_forecast_app_v3.py
...
def reset_from(step: str):
    order = [
        "start","base","festival","base_approval","season","season_approval",
        "pulse","pulse_approval","add_info","foresight","exec","email"
    ]
    idx = order.index(step)
    if idx <= order.index("base"):
        for k in ["base_ran","festival_ran","base_approved","base_human_confirmed","human_adj_df"]:
...
ROW_H = {
    1: 420,  # calendar + chart
    2: 140,  # base table
    3: 140,  # festival table
    4: 160,  # final forecast table
    5: 140,  # season header
    6: 160,  # season approve/tweak result
    7: 140,  # pulse header
    8: 140,  # pulse approve/comment result
    9: 120,  # add info prompt
    10: 180, # foresight results
    11: 180, # executive summary
    12: 120, # email block
}
...
# ---------- LEFT: INPUT / CONTROLS
with left:
    st.markdown("### AGENT FORESIGHT UI")
    st.markdown('<div class="bubble">1) Do you want to start Forecast?</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    if c1.button("YES ✅", key="start_yes"):
        st.session_state["started"] = True
    if c2.button("NO ❌", key="start_no"):
        st.session_state["started"] = False
        reset_from("start")

    if st.session_state["started"]:
        st.markdown('<div class="bubble">2) Agent Base Forecast Triggered</div>', unsafe_allow_html=True)
        ...
        if st.session_state["base_ran"]:
            st.markdown('<div class="bubble">3) Agent Festivalcast Triggered</div>', unsafe_allow_html=True)
        ...
        if st.session_state["festival_ran"]:
            st.markdown('<div class="bubble">4) Do you approve the forecast?</div>', unsafe_allow_html=True)
            c3, c4 = st.columns(2)
            if c3.button("YES ✅ (Base/Fest)", key="approve_base"):
                st.session_state["base_approved"] = True
                st.session_state["human_adj_df"] = spec_human_df.copy()
            if c4.button("NO, add human adj ✍️", key="reject_base"):
        ...
        if can_go_season:
            st.markdown('<div class="bubble">5) Seasoncast is triggered</div>', unsafe_allow_html=True)
        ...
        if st.session_state["season_ran"]:
            st.markdown('<div class="bubble">6) Do you approve the seasonal f’cast?</div>', unsafe_allow_html=True)
            c5, c6 = st.columns(2)
            if c5.button("YES ✅ (Season)", key="approve_season"):
                st.session_state["season_approved"] = True
                st.session_state["season_df"] = spec_season_df.copy()
            if c6.button("NO, tweak ✍️", key="reject_season"):
        ...
        if can_go_pulse:
            st.markdown('<div class="bubble">7) Pulsecast is triggered</div>', unsafe_allow_html=True)
        ...
        if st.session_state["pulse_ran"]:
            st.markdown('<div class="bubble">8) Do you approve the Pulse f’cast?</div>', unsafe_allow_html=True)
            c7, c8 = st.columns(2)
            if c7.button("YES ✅ (Pulse)", key="approve_pulse"):
                st.session_state["pulse_approved"] = True
                st.session_state["pulse_df"] = spec_pulse_df.copy()
            if c8.button("NO, comment ✍️", key="reject_pulse"):
        ...
        if can_add_info:
            st.markdown("---")
            st.markdown('<div class="bubble">9) Add any additional info before Foresight?</div>', unsafe_allow_html=True)
            c9, c10 = st.columns(2)
            if c9.button("YES, add context 🧩", key="add_info_yes"):
                st.session_state["add_info_done"] = True
            if c10.button("NO", key="add_info_no"):
            ...
            if st.session_state["add_info_done"]:
                ...
                st.markdown('<div class="bubble">10) Trigger Foresight</div>', unsafe_allow_html=True)
                ...
                if st.session_state["foresight_ran"]:
                    st.markdown('<div class="bubble">11) Generate executive summary?</div>', unsafe_allow_html=True)
                    c11, c12 = st.columns(2)
                    if c11.button("YES, generate 📝", key="exec_yes"):
                    ...
                    if c12.button("NO", key="exec_no"):
                        st.session_state["exec_summary_toggle"] = False

                    st.markdown('<div class="bubble">12) Email the report?</div>', unsafe_allow_html=True)
                    c13, c14 = st.columns(2)
                    if c13.button("YES, email 📧", key="email_yes"):
                        st.session_state["email_toggle"] = True
                    if c14.button("NO", key="email_no"):
...
with right:
    ...
    # Pre-declare 12 "row" containers so each output aligns with its left-step position.
    # If a step hasn't been reached, we drop a transparent spacer with a min-height.
    r = [st.container() for _ in range(12)]

    # Row 1 — shows only after step 1 YES
    with r[0]:
        if st.session_state["started"]:
            st.markdown('<div class="bubble system captionline">CALENDAR (HOLIDAYS MARKED)</div>', unsafe_allow_html=True)
    ...
    # Row 4 — Final forecast after approval or confirmed human adj
    with r[3]:
